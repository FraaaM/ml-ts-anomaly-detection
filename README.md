...
|----|----------|----------|----------------|-------------|--------------|------------|
| **R1** | Combined IT-Ops | YAHOO, SMD, IOPS, Exathlon, WSD, NEK | univariate series | 1,500 – 20,000 | 500 – 1,500 | Веб-трафик, метрики серверов, логи Spark, сетевые потоки |
...
| Потребление памяти | Высокое (12.9 млн × 34 признака) | Низкое (~2000 серий × 12 признаков) |
...
class Config:
    ...
    lags: Tuple[int, ...] = (1, 5)
    train_sample_frac: float = 1.0  # для тестов на слабом железе
...
4. ✅ Получены солидные метрики: ROC-AUC = 0.89, Recall = 53% при дисбалансе 5%
